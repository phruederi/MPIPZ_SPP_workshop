...
design.file <- paste(results.dir, "design_diversity.txt", sep="")
...
soil_batch_samples <- design$SampleID[design$soil %in% soil]
...
idx <- rownames(bray_curtis) %in% colnames(otu_table)
...
colors <- data.frame(group=c("pooled_nodules", "rhizosphere", "root", "soil"),
                     color=c(c_blue, c_dark_red, c_very_dark_green, c_dark_brown))
...
ggsave(paste(figures.dir, "PCoA_BC_test.pdf", sep=""), p)
